...
from misc import (get_speed, is_within_distance,
                               ...
                               compute_distance, distance_vehicle)
...
class BasicAgent(object):
    ...
    def __init__(self, vehicle, opt_dict={}, map_inst=None, grp_inst=None):
        ...
        if grp_inst:
            if isinstance(grp_inst, GlobalRoutePlanner):
                ...
                print("Warning: Ignoring the given map as it is not a 'carla.Map'")
                self._global_planner = GlobalRoutePlanner(self._map, 0.5)
        else:
            self._global_planner = GlobalRoutePlanner(self._map, 0.5)
    ...
    def _vehicle_obstacle_detected(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        ...
        vehicle_list.sort(
            key=lambda vehicle: distance_vehicle(ego_wpt, vehicle.get_transform())
        )
        ...
        for target_vehicle in vehicle_list:
            ...
            if not ego_wpt.is_junction or not target_wpt.is_junction:
                if not ego_wpt.is_junction and target_wpt.is_junction:
                    pass
                
                elif target_wpt.road_id != ego_wpt.road_id or target_wpt.lane_id != ego_wpt.lane_id  + lane_offset:
                    next_wpt = self._local_planner.get_incoming_waypoint_and_direction(steps=3)[0]
                    if not next_wpt:
                        continue
                    if target_wpt.road_id != next_wpt.road_id or target_wpt.lane_id != next_wpt.lane_id + lane_offset:
